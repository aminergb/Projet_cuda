    ...
    if (j >= paddingSize && j < paddedW - paddingSize && i >= paddingSize && i < paddedH - paddingSize) {
        ...
        for (int k = -paddingSize; k <= paddingSize; k++) {
            ...
            for (int l = -paddingSize; l <= paddingSize; l++) {
                ...
                channelB += (rgb_b[iPixelPos] * Filter[coefPos]) / 273;
                channelG += (rgb_g[iPixelPos] * Filter[coefPos]) / 273;
                channelR += (rgb_r[iPixelPos] * Filter[coefPos]) / 273;
...
int main()
{
    ...
    cv::Mat m_in = cv::imread("C:/Users/lagab/Pictures/hommage_idir1.jpg", cv::IMREAD_COLOR);
    ...
    char mask_h[25] = { 1, 4, 7, 4, 1,
                        4,16, 26, 16, 4,
                        7,26,41, 26,7,
                        4,16,26,16,4,
                        1, 4, 7, 4, 1 };
    HANDLE_ERROR(cudaMemcpyToSymbol(Filter, mask_h, 25));
