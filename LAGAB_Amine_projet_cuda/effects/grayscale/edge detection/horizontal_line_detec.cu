    ...
    if (j >= paddingSize && j < paddedW - paddingSize && i >= paddingSize && i < paddedH - paddingSize) {
        ...
        for (int k = -paddingSize; k <= paddingSize; k++) {
            for (int l = -paddingSize; l <= paddingSize; l++) {
                ...
                channel += (rgb[iPixelPos] * Filter_D[coefPos]);
...
int main()
{
    cv::Mat m_in = cv::imread("images/linehv.png", cv::IMREAD_GRAYSCALE);
    ...
    /* { 1, 4, 7, 4, 1,
                         ...
                         4,16,26,16,4,
                         1, 4, 7, 4, 1 };*/
    char Filter_h[9] = { -1, -1, -1, 2, 2,2,-1,
                         -1,-1, };

    HANDLE_ERROR(cudaMemcpyToSymbol(Filter_D, Filter_h, 9));
  ...
  //  while (count <= 1000) {
    ...
    cv::imshow("f", m_out);
    cv::waitKey();
    cv::imwrite("C:/Users/lagab/Pictures/Lennrer5.jpg", m_out);
