...
__global__ void Emboss(uchar* rgb, uchar* sortie, std::size_t cols, std::size_t rows, uchar paddingSize) {
...
int main()
{
    cv::Mat m_in = cv::imread("images/hommage_idir.jpg", cv::IMREAD_GRAYSCALE);
    ...
    //while (count <= 1000) {
        ...
        Emboss<< < b, t >> > (sob_h, sob_d, cols, rows, S);
  ...
  //  }
    ...
    cv::imshow("f", m_out);
    cv::waitKey();
    cv::imwrite("C:/Users/lagab/Pictures/hommage_idir_emboss.jpg", m_out);
