...
__constant__ char FilterH_D[9];
__constant__ char FilterV_D[9];

__global__ void Sobel(uchar* rgb, uchar* sortie, std::size_t cols, std::size_t rows, uchar paddingSize) {
    ...
    if (j >= paddingSize && j < paddedW - paddingSize && i >= paddingSize && i < paddedH - paddingSize) {
        ...
        for (int k = -paddingSize; k <= paddingSize; k++) {
            for (int l = -paddingSize; l <= paddingSize; l++) {
                ...
                hh += (rgb[iPixelPos] * FilterH_D[coefPos]);
                vv += (rgb[iPixelPos] * FilterV_D[coefPos]);

            }

        }
        auto vvhh = hh * hh + vv * vv;
        vvhh = vvhh > 255 * 255 ? vvhh = 255 * 255 : vvhh;
...
int main()
{
    ...
    char Filter_h[9] = { -1, 0, 1,
                        -2, 0, 2,
                        -1, 0, 1 };

    char Filter_v[9] = { -1, -2, -1,
                         0, 0, 0,
                         1, 2, 1 };

    HANDLE_ERROR(cudaMemcpyToSymbol(FilterH_D, Filter_h, 9));
    HANDLE_ERROR(cudaMemcpyToSymbol(FilterV_D, Filter_v, 9));
    dim3 t(32, 32);
    ...
    while (count <= 1000) {
        ...
        Sobel << < b, t >> > (sob_h, sob_d, cols, rows, S);
