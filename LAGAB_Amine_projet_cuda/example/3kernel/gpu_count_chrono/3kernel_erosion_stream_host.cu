    ...
    if (j >= paddingSize && j < paddedW - paddingSize && i >= paddingSize && i < paddedH - paddingSize) {
        ...
        for (int k = -paddingSize; k <= paddingSize; k++) {
            for (int l = -paddingSize; l <= paddingSize; l++) {
                ...
                channel =min(channel , rgb[iPixelPos]);
            }
...
int main()
{
    cv::Mat m_in = cv::imread("C:/Users/lagab/Pictures/draw.jpg", cv::IMREAD_COLOR);
...
char mask_h[9] = { 1,1, 1,2 , 2, 2, 2, 1, 1 };
   ...
   // while (count <= 1000) {
        ...
        HANDLE_ERROR(cudaMalloc(&blue_h, rows_bo * cols_bo), streamblue);
        HANDLE_ERROR(cudaMalloc(&green_h, rows_bo * cols_bo), streamgreen);
        HANDLE_ERROR(cudaMalloc(&red_h, rows_bo * cols_bo), streamred);

        HANDLE_ERROR(cudaHostAlloc((void**)&blue_d, rows * cols * sizeof(*blue_d), cudaHostAllocDefault), streamblue);
        HANDLE_ERROR(cudaHostAlloc((void**)&green_d, rows * cols * sizeof(*green_d), cudaHostAllocDefault), streamgreen);
        HANDLE_ERROR(cudaHostAlloc((void**)&red_d, rows * cols * sizeof(*red_d), cudaHostAllocDefault), streamred);
        ...
        HANDLE_ERROR(cudaFree(blue_h), streamblue);
        HANDLE_ERROR(cudaFree(green_h), streamgreen);
        HANDLE_ERROR(cudaFree(red_h), streamred);
